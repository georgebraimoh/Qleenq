...
const QleenqContext = createContext();

export function QleenqProvider({ children }) {
  ...
  const createHangout = async (newHangoutData) => {
    ...
    try {
      const welcomeMsg = await hangoutService.sendSpaceMessage({
        ...
        userAvatar: currentUser.avatar,
        text: `${currentUser.name || 'Host'} created the Hangout and opened the Qleenq Space!`,
        type: 'system'
      });
  ...
  return (
    <QleenqContext.Provider value={{
      ...
      {children}
    </QleenqContext.Provider>
  );
}

export function useQleenq() {
  return useContext(QleenqContext);
}
